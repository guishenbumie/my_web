...
import com.sfm.etcd.api.RequestOp;
import com.sfm.etcd.api.TxnRequest;
import com.sfm.etcd.impl.KVClient;
import com.sfm.etcd.options.DeleteOption;
import com.sfm.etcd.options.GetOption;
import com.sfm.etcd.options.PutOption;
...
public abstract class Op {
    ...
    public static final class PutOp extends Op {
        ...
        RequestOp toRequestOp(ByteSequence namespace) {
            return RequestOp.newBuilder()
                    .setRequestPut(KVClient.mapPutRequest(ByteSequence.from(key), ByteSequence.from(value), option, namespace))
                    .build();
        }
    }
    ...
    public static final class GetOp extends Op {
        ...
        RequestOp toRequestOp(ByteSequence namespace) {
            return RequestOp.newBuilder()
                    .setRequestRange(KVClient.mapRangeRequest(ByteSequence.from(key), option, namespace))
                    .build();
        }
    }
    ...
    public static final class DeleteOp extends Op {
        ...
        RequestOp toRequestOp(ByteSequence namespace) {
            return RequestOp.newBuilder()
                    .setRequestDeleteRange(KVClient.mapDeleteRequest(ByteSequence.from(key), option, namespace))
                    .build();
        }
    }
